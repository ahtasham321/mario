...
export class Move implements Power {
  ...
  public update(time: number, delta: number, player: Player, cursors: Phaser.Types.Input.Keyboard.CursorKeys) {
    ...
    if (cursors.left.isDown) {
      player.setFlipX(false) // flip left to right
      player.body.setAccelerationX(ax + (velocity.x < 0 ? -velocity.x * 2 : 0)) // set acceleration to move right
    } else if (cursors.right.isDown) {
      player.setFlipX(true) // flip right to left
      player.body.setAccelerationX(-ax - (velocity.x > 0 ? velocity.x * 2 : 0)) // set acceleration to move left
    } else {
